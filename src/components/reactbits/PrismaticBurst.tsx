...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uNoiseAmount: { value: Math.max(0, Math.min(1, noiseAmount)) },
        uRayCount: { value: 0 }
      }
    });
    ...
    const update = (now: number) => {
      ...
      const nextMouse: [number, number] = [sm[0], sm[1]];
      program.uniforms.uMouse.value = nextMouse;
  ...
  useEffect(() => {
    ...
    program.uniforms.uNoiseAmount.value = Math.max(0, Math.min(1, noiseAmount));
